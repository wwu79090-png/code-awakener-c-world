...
assert(/World Build v1\.0\.6/m.test(html) || /GAME_VERSION\s*=\s*"v1\.0\.6"/m.test(html), "game version should increment when shipping a new update");
...
assert(/GAME_VERSION\s*=\s*"v1\.0\.6"/m.test(html), "game version should increment for the absolute guide release");
...
assert(/UPDATE_ANNOUNCEMENT_PAGES\s*=\s*Object\.freeze\(\[[\s\S]*绝对引导系统[\s\S]*前10分钟[\s\S]*错误行标红/m.test(html), "startup announcement should describe the current v1.0.6 update");
...
assert(/id="programmingPassportPanel"/m.test(html) && /function renderProgrammingPassport/m.test(html) && /function saveProgrammingPassportImage/m.test(html), "programming passport should be visible in the side menu and saveable");
assert(/id="fillCurrentLessonButton"/m.test(html) && /function fillCurrentLessonCode/m.test(html) && /order\s*<=\s*10/m.test(html), "first ten lessons should support click-to-fill coding");
assert(/CODE_TEMPLATE_LIBRARY\s*=\s*Object\.freeze/m.test(html) && /id="codeTemplateLibrary"/m.test(html) && /function insertCodeTemplate/m.test(html), "code template library should allow one-click insertion");
assert(/id="executionSpeedSlider"/m.test(html) && /function getExecutionSpeedMultiplier/m.test(html), "execution animation speed should be adjustable independently from text speed");
assert(/id="executionPauseButton"/m.test(html) && /id="executionStepButton"/m.test(html) && /function pauseExecutionVisualization/m.test(html) && /function stepExecutionVisualization/m.test(html), "execution visualization should support pause and step controls");
assert(/function generateKnowledgeCard/m.test(html) && /id="knowledgeCardButton"/m.test(html), "completed lessons should be able to generate a knowledge card");
assert(/id="exportProgressButton"/m.test(html) && /id="importProgressInput"/m.test(html) && /function exportLearningProgress/m.test(html) && /function importLearningProgress/m.test(html), "progress export/import should be available from the side menu");
...
assert(/data-world-difficulty="easy"/m.test(html) && /data-world-difficulty="normal"/m.test(html) && /data-world-difficulty="hard"/m.test(html), "side menu should expose easy/normal/hard difficulty buttons");
...
assert(/id="heroAvatarCanvas"/m.test(html) && /id="regenerateHeroAvatarButton"/m.test(html), "side menu should show a high-resolution hero avatar with regenerate control");
assert(/id="worldEvolutionChronicleText"/m.test(html), "side menu should show the world evolution chronicle diary");
assert(/id="worldEvolutionAchievementList"/m.test(html), "side menu should render achievement unlock states");
assert(/id="resetWorldEvolutionButton"/m.test(html) && /function resetWorldEvolutionProgress/m.test(html), "side menu should include reset world progress while preserving achievements");
...
assert(/当前任务[\s\S]*新手学习[\s\S]*记录收藏[\s\S]*世界与角色[\s\S]*设置备份/m.test(html), "side menu should expose the five simplified groups");
