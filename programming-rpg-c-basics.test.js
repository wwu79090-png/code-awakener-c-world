...
assert(/UPDATE_ANNOUNCEMENT_PAGES\s*=\s*Object\.freeze\(\[\s*\{\s*title:\s*"> 消息 \/ 本次更新"[\s\S]*v1\.2\.0-hotfix\.7[\s\S]*大号“点击运行”引导条[\s\S]*最后一句“点击运行”旁白[\s\S]*4\.2 秒[\s\S]*file:\/\/[\s\S]*printf 字符级解构[\s\S]*414374792/m.test(html), "startup announcement should show the current guided run proxy and TTS synchronization hotfix before upload");
assert(/id="announcementPageBody"[\s\S]*v1\.2\.0-hotfix\.7[\s\S]*大号“点击运行”引导条[\s\S]*最后一句“点击运行”旁白[\s\S]*4\.2 秒[\s\S]*file:\/\/[\s\S]*printf 字符级解构[\s\S]*官方Q群：414374792/m.test(initialBodyMarkup), "static startup announcement placeholder should match the current guided run proxy and TTS synchronization hotfix before script hydration");
...
assert(/UPDATE_HISTORY\s*=\s*Object\.freeze\(\[\s*\{[\s\S]*v1\.2\.0-hotfix\.7[\s\S]*点击运行代理与 TTS 同步热修[\s\S]*大号“点击运行”引导条[\s\S]*最后一句“点击运行”旁白[\s\S]*4\.2 秒[\s\S]*file scheme unsupported[\s\S]*未改动 Pixi 版文件[\s\S]*官方Q群：414374792/m.test(html), "update history should record the current guided run proxy and TTS synchronization hotfix");
...
assert(/setCompilerHandsOnSubtitle\("finish"\)[\s\S]*const finishWait = waitForCompilerCabinHandsOnNarrationStage\(session, "finish", chapter\)[\s\S]*dom\.runButton\.disabled = false[\s\S]*focusButtonWithVisualPulse\(dom\.runButton\)/m.test(html), "guided Run should become clickable only after the final click-run narration finishes");
...
assert(/\.compiler-hands-on-subtitle\[data-focus="run"\] \{[\s\S]*pointer-events:\s*auto[\s\S]*cursor:\s*pointer/m.test(html) && /function activateCompilerHandsOnRunFromGuide\(event\)[\s\S]*dataset\?\.focus !== "run"[\s\S]*dom\.runButton\.click\(\)[\s\S]*dom\.compilerHandsOnSubtitle\?\.addEventListener\("click", activateCompilerHandsOnRunFromGuide\)/m.test(html), "clicking the large guided Run step should proxy to the real Run button instead of feeling stuck");
