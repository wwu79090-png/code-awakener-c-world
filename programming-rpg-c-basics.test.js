...
const learningCabinShellStart = html.indexOf(".learning-cabin-editor-shell {");
const learningCabinBottomStart = html.indexOf(".learning-cabin-editor-bottom {");
const learningCabinShellCss = html.slice(learningCabinShellStart, html.indexOf(".vscode-window {", learningCabinShellStart));
const learningCabinBottomCss = html.slice(learningCabinBottomStart, html.indexOf(".learning-cabin-editor-output {", learningCabinBottomStart));
const learningCabinScrollbarCss = html.slice(html.indexOf(":where(.menu-list"), html.indexOf("input[type=\"range\"]"));
const editorCenterStart = html.indexOf(".editor-center {", html.indexOf(".file-tree {"));
const editorCenterCss = html.slice(editorCenterStart, html.indexOf(".tabs {", editorCenterStart));
const editorProLayerStart = html.indexOf(".editor-pro-layer {", editorCenterStart);
const editorMinimapStart = html.indexOf(".editor-minimap {", editorProLayerStart);
const editorProLayerCss = html.slice(editorProLayerStart, editorMinimapStart);
const editorMinimapCss = html.slice(editorMinimapStart, html.indexOf(".editor-notes-panel,", editorMinimapStart));
assert(html.includes('id="editorOverlay" class="learning-cabin-editor-overlay"'), "editor overlay should use the reusable learning-cabin overlay shell");
assert(html.includes('class="vscode-window pixel-panel learning-cabin-editor-shell" role="dialog" aria-label="内置代码编辑器" data-learning-cabin-shell="first-world-code-gate"'), "first-world code gate should opt into the reusable learning-cabin editor shell");
assert(html.includes('class="editor-main learning-cabin-editor-scroll"') && html.includes('class="console learning-cabin-editor-bottom"') && html.includes('class="console-actions learning-cabin-editor-actions"'), "learning-cabin shell should mark scrollable content and persistent bottom actions");
assert(learningCabinShellCss.includes("grid-template-rows: var(--learning-cabin-shell-rows, 34px minmax(0, 1fr) minmax(156px, 24%));") && learningCabinShellCss.includes("max-height: calc(100dvh - var(--editor-safe-top) - var(--editor-safe-bottom));") && learningCabinShellCss.includes("overflow: hidden;"), "reusable learning-cabin shell should constrain itself to safe-area-aware viewport bounds");
assert(learningCabinBottomCss.includes("grid-template-rows: minmax(var(--learning-cabin-action-min-height, 40px), auto) minmax(var(--learning-cabin-hint-min-height, 34px), auto) minmax(0, 1fr);") && learningCabinBottomCss.includes("overflow: hidden;") && html.includes(".learning-cabin-editor-actions {\n      position: sticky;\n      top: 0;\n      z-index: 5;"), "reusable learning-cabin bottom region should keep action controls pinned above scrollable output");
assert([".learning-cabin-editor-scroll", ".learning-cabin-editor-pane", ".learning-cabin-editor-output", "scrollbar-color: var(--hud-scroll-thumb) var(--hud-scroll-track);"].every((marker) => learningCabinScrollbarCss.includes(marker)), "learning-cabin shell scroll regions should inherit the themed HUD scrollbar system");
assert(editorCenterCss.includes("grid-template-rows: 34px minmax(0, 1fr) minmax(0, 90px);") && editorProLayerCss.includes("max-height: 90px;") && editorProLayerCss.includes("overflow: hidden;") && editorMinimapCss.includes("max-height: 74px;"), "editor center should keep long-code minimap chrome from collapsing the main input viewport");
...
assert(typeof api.handleNpcInteraction === "function", "NPC interaction entrypoint should be exported for crash regression tests");
{
  ...
  assert(api.getNearestNpcInteraction({ player: { x: 10, y: 10 }, coreNpcs: [{ data: { id: "destroyed" }, container: { x: 10, y: 10, destroyed: true } }] }) === null, "nearest NPC lookup should ignore destroyed NPC containers instead of reopening stale interactions");
...
assert(/function updateNpcGazeTracking\(scene\)[\s\S]*getNpcWorldPoint\(npc\)[\s\S]*typeof target\.rotation !== "number"[\s\S]*target\.rotation = 0/m.test(html), "NPC gaze updates should skip stale NPCs and normalize missing rotation instead of crashing the frame");
assert(/function updateNpcQuestIndicators\(scene\)[\s\S]*if \(!npc\?\.data\) return/m.test(html), "NPC quest indicator updates should skip invalid NPC entries");
assert(/getInteraction\(\)[\s\S]*const nearestNpc = getNearestNpcInteraction\(this\);[\s\S]*return \{ type: "npc", npc: nearestNpc \}/m.test(html), "keyboard and touch interaction scanning should include the nearest safe NPC runtime entry");
assert(/performCurrentInteraction\(source = "keyboard"[\s\S]*interaction\?\.type === "npc"[\s\S]*SafeExecute\.run\([\s\S]*source: "npc-current-interaction"/m.test(html), "NPC interaction dispatch should be protected before ripple and scan effects can throw");
...
assert(/function applyCompilerCabinHandsOnCodeProgress\(session[\s\S]*dom\.codeInput\.value = target\.slice\(0, index\)[\s\S]*syncCompilerHandsOnProgress[\s\S]*function playCompilerCabinHandsOnEditorDemo\(chapter[\s\S]*target[\s\S]*applyCompilerCabinHandsOnCodeProgress\(session, session\.index \+ 1\)[\s\S]*focusButtonWithVisualPulse\(dom\.runButton\)/m.test(html), "compiler cabin hands-on demo should type the real task code into the actual editor through a seekable progress state before focusing Run");
...
assert(/id="compilerHandsOnProgress"[\s\S]*type="range"[\s\S]*id="compilerHandsOnJumpRunButton"[\s\S]*跳到运行/m.test(html), "hands-on editor demo should expose a draggable progress control and a jump-to-run button");
...
assert(/compilerHandsOnPendingRunCompleteChapterId[\s\S]*waitForCompilerCabinHandsOnRun\(chapter[\s\S]*compilerHandsOnPendingRunCompleteChapterId === chapterId[\s\S]*return Promise\.resolve\(true\)[\s\S]*notifyCompilerCabinHandsOnRunComplete\(chapterId[\s\S]*compilerHandsOnPendingRunCompleteChapterId = chapterId/m.test(html), "hands-on run completion should be cached if the player clicks before the waiter is installed");
...
assert(/function shouldDeferCompilerCabinAutoComplete\(chapterId[\s\S]*compilerHandsOnRunWaiter[\s\S]*gameState\.compilerCabinLessonActive[\s\S]*deferCompilerCabinAutoComplete[\s\S]*setCompilerHandsOnSubtitle\("output"\)[\s\S]*return result[\s\S]*tutorialAnimationManager\.trigger\("T07_COMPILE_SUCCESS"/m.test(html), "first cabin hands-on success should show output inside the editor before auto-completing the chapter even if the run click races the waiter setup");
...
assert(/function seekCompilerCabinHandsOnProgress\(percent[\s\S]*applyCompilerCabinHandsOnCodeProgress[\s\S]*releaseCompilerCabinHandsOnRunControl\(session, reason\)[\s\S]*dom\.compilerHandsOnProgress\?\.addEventListener\("input", handleCompilerHandsOnProgressInput\)[\s\S]*dom\.compilerHandsOnJumpRunButton\?\.addEventListener\("click", handleCompilerHandsOnJumpRun\)/m.test(html), "hands-on progress slider and jump button should be wired to the same release path as the Run guidance");
assert(/dom\.runButton\.addEventListener\("click"[\s\S]*compilerHandsOnDemoSession[\s\S]*forceCompilerCabinHandsOnRunReady\("run-button-click"\)[\s\S]*notifyCompilerCabinHandsOnRunComplete\(activeChapterId\)/m.test(html) && /function handleEditorRunKey\(event\)[\s\S]*dom\.runButton\.click\(\)/m.test(html), "real Run clicks and Ctrl+Enter should fast-forward an active hands-on demo instead of bypassing the tutorial state machine");
...
assert(/playCompilerCabinLesson\(chapterId\)[\s\S]*isLearned\(chapter\.id\)[\s\S]*openCompilerCabinHandsOnEditor\(chapter,\s*\{\s*preserveExistingCode:\s*true/m.test(html), "re-entering a learned compiler cabin should resume the editor through the guarded handoff without replaying and stacking the cinematic tutorial");
...
assert(/function clearCompilerCabinHandoffBlockers\(reason = "compiler-cabin-handoff"\)[\s\S]*gameState\.dialogOpen = false[\s\S]*closeNpcActionChooser\?\.\(\)[\s\S]*closeNpcCodeWindow\?\.\(\)/m.test(html), "compiler cabin movie handoff should clear stale dialogue and NPC overlays before opening the real editor");
assert(/function openCompilerCabinHandsOnEditor\(chapter,[\s\S]*cancelCompilerCabinHandsOnEditorDemo\("compiler-cabin-handoff"\)[\s\S]*openEditor\(chapterId,[\s\S]*refreshEditorDeviceSymbolAssist\(document\.body\.classList\.contains\("mobile-input"\)\)[\s\S]*gameState\.editorOpen && gameState\.activeChallengeId === chapterId/m.test(html), "compiler cabin handoff should atomically open the editor and refresh mobile symbol buttons or PC English punctuation hints");
assert(/function seedCompilerCabinHandsOnEditorDemo\(session\)[\s\S]*target\.startsWith\(existing\)[\s\S]*applyCompilerCabinHandsOnCodeProgress\(session, existing\.length/m.test(html), "compiler cabin hands-on demo should continue compatible existing editor code instead of resetting it during handoff");
assert(/openCompilerCabinHandsOnEditor\(chapter,[\s\S]*playCompilerCabinHandsOnEditorDemo\(chapter\)[\s\S]*playPrintfCharacterDeconstruction\(getCompilerCabinDeconstructionCode\(chapter\)\)/m.test(html), "compiler cabin should enter the editor through the guarded real hands-on demo instead of a static side-panel-only explanation");
...
assert(/MenuManager[\s\S]*enterGameWorld[\s\S]*maybePromptMobileFullscreen/m.test(html), "entering the game world should schedule the mobile landscape fullscreen prompt");
{
  ...
  assert(/#postGenesisSetupConfirmButton[\s\S]*button\.evaluate[\s\S]*element\.click\(\)/m.test(mobileSmokeScript), "mobile smoke should activate post-genesis confirmation inside the page so a successful cinematic handoff is not reported as a Playwright click timeout");
