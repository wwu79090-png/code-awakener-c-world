...
assert(/UPDATE_ANNOUNCEMENT_PAGES\s*=\s*Object\.freeze\(\[\s*\{\s*title:\s*"> 消息 \/ 本次更新"[\s\S]*v1\.2\.0-hotfix\.4[\s\S]*1-6 顺序轨[\s\S]*单焦点动线[\s\S]*ChatTTS[\s\S]*speaker embedding[\s\S]*printf \/ Hello World \/ include \/ int main[\s\S]*--force-keys[\s\S]*414374792/m.test(html), "startup announcement should show the current cinematic rail and ChatTTS hotfix before upload");
assert(/id="announcementPageBody"[\s\S]*v1\.2\.0-hotfix\.4[\s\S]*1-6 顺序轨[\s\S]*单焦点动线[\s\S]*ChatTTS[\s\S]*speaker embedding[\s\S]*printf \/ Hello World \/ include \/ int main[\s\S]*官方Q群：414374792/m.test(initialBodyMarkup), "static startup announcement placeholder should match the current cinematic rail and ChatTTS hotfix before script hydration");
...
assert(/UPDATE_HISTORY\s*=\s*Object\.freeze\(\[\s*\{[\s\S]*v1\.2\.0-hotfix\.4[\s\S]*电影顺序轨与单导师女声热修[\s\S]*visibility hidden[\s\S]*编号焦点动线[\s\S]*ChatTTS[\s\S]*printf、Hello World、include、int main[\s\S]*--force-keys[\s\S]*官方Q群：414374792/m.test(html), "update history should record the current cinematic rail and ChatTTS hotfix");
