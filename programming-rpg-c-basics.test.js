...
assert(/this\.sfxBusScale\s*=\s*0\.72/m.test(html) && /this\.sfxEventGain\s*=\s*0\.68/m.test(html), "SFX calibration should stay clearly audible after burst protection");
...
assert(/World Build v1\.0\.31/m.test(html) || /GAME_VERSION\s*=\s*"v1\.0\.31"/m.test(html), "game version should increment when shipping a new update");
...
assert(/GAME_VERSION\s*=\s*"v1\.0\.31"/m.test(html), "game version should increment for the custom music connector release");
...
assert(/html\.app-rendered \.static-rescue/m.test(html) && /safeMode=true&noAudio=true&v=1\.0\.31/m.test(html), "static rescue should only appear if the app script does not render");
...
assert(/UPDATE_ANNOUNCEMENT_PAGES\s*=\s*Object\.freeze\(\[\s*\{\s*title:\s*"> 消息 \/ 本次更新"[\s\S]*v1\.0\.31[\s\S]*QQ音乐外部模式[\s\S]*自定义场景音乐[\s\S]*414374792/m.test(html), "startup announcement should show only the current v1.0.31 custom music update");
assert(/id="announcementPageBody"[\s\S]*v1\.0\.31：QQ音乐外部模式与自定义场景音乐[\s\S]*官方Q群：414374792/m.test(initialBodyMarkup), "static startup announcement placeholder should match the current v1.0.31 update before script hydration");
...
assert(/UPDATE_ANNOUNCEMENT_PAGES\s*=\s*Object\.freeze\(\[[\s\S]*QQ音乐外部模式[\s\S]*自定义场景音乐[\s\S]*播放\/暂停切换[\s\S]*414374792/m.test(html), "startup announcement should describe the current custom music update");
